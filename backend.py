...
# ============ UNIVERSAL ============
RX_IVA21_ANY = re.compile(r"(IVA[^\n]{0,200}?21,00\s*%)[^\n]{0,80}?([\-−]?\s?\d{1,3}(?:\.\d{3})*,\d{2})", re.IGNORECASE)
RX_IVA21_RI  = re.compile(r"(IVA\s*RI\s*CRED\.?\s*FISC\.?\s*COMERCIO\s*S/DTO\s*F\.?OTORG)[^\d\-−]{0,80}([\-−]?\d{1,3}(?:\.\d{3})*,\d{2})", re.IGNORECASE)
RX_IVA105    = re.compile(r"(IVA\s*CRED\.?\s*FISC\.?\s*COM\.?\s*L\.?\s*25063\s*S/DTO\s*F\.?OTOR\s*10,50%|IVA\s*S/COSTO\s*FINANCIERO\s*10,50%)" r"[^\d\-−]*(\-?\d{1,3}(?:\.\d{3})*,\d{2})", re.IGNORECASE)
RX_PERC_IVA_30 = re.compile(r"PERCEPCI[ÓO]N\s*IVA\s*(?:R\.?\s*G\.?|RG)\s*2408\s*3,00\s*%[^\d\-−]*(\-?\d{1,3}(?:\.\d{3})*,\d{2})", re.IGNORECASE)
RX_PERC_IVA_15 = re.compile(r"PERCEPCI[ÓO]N\s*IVA\s*(?:R\.?\s*G\.?|RG)\s*2408\s*1,50\s*%[^\d\-−]*(\-?\d{1,3}(?:\.\d{3})*,\d{2})", re.IGNORECASE)
...
def extract_universal(text: str) -> dict:
    ...
    for m in RX_IVA21_RI.finditer(text):  tot["iva21"] += to_float_signed(m.group(2))
...
def extract_resumen_from_bytes(pdf_bytes: bytes) -> pd.DataFrame:
    ...
    if saw_cabal:
        iva21   = round(cabal_tot["iva_arancel"], 2)  # respeta tu app Cabal
...
def build_report_pdf(resumen_df: pd.DataFrame, out_path: str, titulo: str, agregar_total_general: bool = True):
    ...
    doc = SimpleDocTemplate(out_path, pagesize=A4); story = []
    story.append(Paragraph(titulo, title_style))
    story.append(Paragraph(f"Generado: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", normal))
    story.append(Spacer(1, 12)); story.append(Paragraph("Resumen de importes", h2))
    data = [["Concepto", "Monto ($)"]]; total_general = 0.0
    for _, row in resumen_df.iterrows():
        val = float(row["Monto Total"]); total_general += val
    ...
    tbl.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,0), "#222"),
        ("TEXTCOLOR", (0,0), (-1,0), "whitesmoke"),
        ("FONTNAME", (0,0), (-1,0), "Helvetica-Bold"),
        ("ALIGN", (1,1), (-1,-1), "RIGHT"),
        ("GRID", (0,0), (-1,-1), 0.25, "grey"),
        ("ROWBACKGROUNDS", (0,1), (-1,-2), ["#f7f7f7", "white"]),
        ("BACKGROUND", (0,-1), (-1,-1), "#e6f2ff"),
        ("FONTNAME", (0,-1), (-1,-1), "Helvetica-Bold"),
    ]))
    doc.build(story := story + [tbl])
    return out_path
